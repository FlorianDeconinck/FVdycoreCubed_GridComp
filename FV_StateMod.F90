...
#include "mpif.h"
   ...
   ! SERIALBOX-BLOCK
#ifdef SERIALIZE
integer :: mpi_rank,ier, timestep_len, timestep_status, save_timestep
character(len=256) :: ser_env, save_timestep_str
#endif
   !#######################################################################
...
contains
   ...
   subroutine FV_Setup(GC,LAYOUT_FILE, RC)
      ...
      !#######################################################################
#ifdef SERIALIZE
call get_environment_variable("SER_ENV", ser_env)
call get_environment_variable("SAVE_TIMESTEP", save_timestep_str, timestep_len, timestep_status)
if (timestep_status .eq. 0 .and. timestep_len .gt. 0) then
read (save_timestep_str,*) save_timestep
else
save_timestep = 1
endif
call mpi_comm_rank(MPI_COMM_WORLD, mpi_rank,ier)
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #772
PRINT *, '>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<'
...
call ppser_initialize( &
           ...
           mpi_rank=mpi_rank, &
           unique_id=.true.)
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #773
call ppser_set_mode(0)
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #774
   ...
   subroutine FV_Run (STATE, EXPORT, CLOCK, GC, RC)
      ...
      select case ( FV_Atm(1)%flagstruct%nwat )
       ...
       case (6:7)
         ...
         grpl = 6
         qcld = 7
         ! Advect around CN fractions of species for redistribution...
         qlcnf = 8
         qicnf = 9
         clcnf = 10
       case (3)
      ...
      if (.not. ADIABATIC) then
         ...
         if (FV_Atm(1)%flagstruct%nwat > 0) then
            do n=1,STATE%GRID%NQ
               ...
               if (TRIM(state%vars%tracer(n)%tname) == 'QLCN') then
                  ...
                  if (qlcnf /= -1) then   ! QLCN
                     ...
                     nn = nn+1
                     if (state%vars%tracer(n)%is_r4) then
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qlcnf) = state%vars%tracer(n)%content_r4(:,:,:)
                     else
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qlcnf) = state%vars%tracer(n)%content(:,:,:)
                     endif
                  endif
               endif
               ...
               if (TRIM(state%vars%tracer(n)%tname) == 'QICN') then
                  ...
                  if (qicnf /= -1) then   ! QICN
                     ...
                     nn = nn+1
                     if (state%vars%tracer(n)%is_r4) then
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qicnf) = state%vars%tracer(n)%content_r4(:,:,:)
                     else
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qicnf) = state%vars%tracer(n)%content(:,:,:)
                     endif
                  endif
               endif
               ...
               if (TRIM(state%vars%tracer(n)%tname) == 'CLCN') then
                  ...
                  if (clcnf /= -1) then   ! CLCN
                     ...
                     nn = nn+1
                     if (state%vars%tracer(n)%is_r4) then
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,clcnf) = state%vars%tracer(n)%content_r4(:,:,:)
                     else
                        FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,clcnf) = state%vars%tracer(n)%content(:,:,:)
                     endif
                  endif
               endif
            ...
            enddo
            ! make convective fractions for advection
            if ( (qliq /= -1) .and. (qlcnf /= -1) ) then
               do k=1,npz
                  do j=jsc,jec
                     do i=isc,iec
                        if (FV_Atm(1)%q(i,j,k,qliq) > 0.0) then
                           FV_Atm(1)%q(i,j,k,qlcnf) = FV_Atm(1)%q(i,j,k,qlcnf) / &
                              FV_Atm(1)%q(i,j,k,qliq)
                        else
                           FV_Atm(1)%q(i,j,k,qlcnf) = 0.0
                        endif
                     enddo
                  enddo
               enddo
            endif
            if ( (qice /= -1) .and. (qicnf /= -1) ) then
               do k=1,npz
                  do j=jsc,jec
                     do i=isc,iec
                        if (FV_Atm(1)%q(i,j,k,qice) > 0.0) then
                           FV_Atm(1)%q(i,j,k,qicnf) = FV_Atm(1)%q(i,j,k,qicnf) / &
                              FV_Atm(1)%q(i,j,k,qice)
                        else
            ...
            select case (FV_Atm(1)%flagstruct%nwat)
             case (6:7)
               ...
               _ASSERT(QLCN_FILLED, 'QLCNF Not Filled')
               _ASSERT(QICN_FILLED, 'QICNF Not Filled')
               _ASSERT(CLCN_FILLED, 'CLCNF Not Filled')
...
#ifdef SERIALIZE
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2033
call fs_enable_serialization()
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2034
call fs_create_savepoint('FVDynamics-In', ppser_savepoint)
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2035
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'bdt', myDT)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bdt', myDT)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bdt', myDT, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2036
SELECT CASE ( ppser_get_mode() )
  CASE(0)
  ...
  CASE(2)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2037
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2038
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2039
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ks', FV_Atm(1)%ptop)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ks', FV_Atm(1)%ptop)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ks', FV_Atm(1)%ptop, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2040
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ncnst', FV_Atm(1)%ncnst)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ncnst', FV_Atm(1)%ncnst)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ncnst', FV_Atm(1)%ncnst, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2041
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'n_split', state%nsplit)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'n_split', state%nsplit)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'n_split', state%nsplit, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2042
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'u', FV_Atm(1)%u)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', FV_Atm(1)%u)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', FV_Atm(1)%u, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2043
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'v', FV_Atm(1)%v)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', FV_Atm(1)%v)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', FV_Atm(1)%v, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2044
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'w', FV_Atm(1)%w)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'w', FV_Atm(1)%w)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'w', FV_Atm(1)%w, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2045
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'delz', FV_Atm(1)%delz)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delz', FV_Atm(1)%delz)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delz', FV_Atm(1)%delz, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2046
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pt', FV_Atm(1)%pt)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', FV_Atm(1)%pt)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', FV_Atm(1)%pt, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2047
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'delp', FV_Atm(1)%delp)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', FV_Atm(1)%delp)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', FV_Atm(1)%delp, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2048
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2049
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2050
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2051
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2052
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2053
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2054
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2055
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ps', FV_Atm(1)%ps)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ps', FV_Atm(1)%ps)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ps', FV_Atm(1)%ps, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2056
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe', FV_Atm(1)%pe)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe', FV_Atm(1)%pe)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe', FV_Atm(1)%pe, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2057
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pk', FV_Atm(1)%pk)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pk', FV_Atm(1)%pk)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pk', FV_Atm(1)%pk, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2058
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'peln', FV_Atm(1)%peln)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', FV_Atm(1)%peln)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', FV_Atm(1)%peln, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2059
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pkz', FV_Atm(1)%pkz)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pkz', FV_Atm(1)%pkz)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pkz', FV_Atm(1)%pkz, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2060
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'phis', FV_Atm(1)%phis)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'phis', FV_Atm(1)%phis)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'phis', FV_Atm(1)%phis, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2061
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'q_con', FV_Atm(1)%q_con)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_con', FV_Atm(1)%q_con)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_con', FV_Atm(1)%q_con, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2062
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'omga', FV_Atm(1)%omga)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', FV_Atm(1)%omga)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', FV_Atm(1)%omga, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2063
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ua', FV_Atm(1)%ua)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', FV_Atm(1)%ua)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', FV_Atm(1)%ua, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2064
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'va', FV_Atm(1)%va)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', FV_Atm(1)%va)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', FV_Atm(1)%va, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2065
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'uc', FV_Atm(1)%uc)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'uc', FV_Atm(1)%uc)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'uc', FV_Atm(1)%uc, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2066
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'vc', FV_Atm(1)%vc)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'vc', FV_Atm(1)%vc)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'vc', FV_Atm(1)%vc, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2067
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ak', FV_Atm(1)%ak)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ak', FV_Atm(1)%ak)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ak', FV_Atm(1)%ak, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2068
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'bk', FV_Atm(1)%bk)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bk', FV_Atm(1)%bk)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'bk', FV_Atm(1)%bk, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2069
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2070
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2071
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2072
SELECT CASE ( ppser_get_mode() )
  CASE(0)
...
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2073
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'diss_estd', FV_Atm(1)%diss_est)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'diss_estd', FV_Atm(1)%diss_est)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'diss_estd', FV_Atm(1)%diss_est, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2074
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'consv_te', FV_Atm(1)%flagstruct%consv_te)
  CASE(1)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'consv_te', FV_Atm(1)%flagstruct%consv_te)
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'consv_te', FV_Atm(1)%flagstruct%consv_te, ppser_zrperturb)
END SELECT
#endif
            call fv_dynamics( &
               ...
               time_total)
#ifdef SERIALIZE
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2093
call fs_create_savepoint('FVDynamics-Out', ppser_savepoint)
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2094
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'u', FV_Atm(1)%u)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'u', FV_Atm(1)%u, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2095
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'v', FV_Atm(1)%v)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'v', FV_Atm(1)%v, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2096
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'w', FV_Atm(1)%w)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'w', FV_Atm(1)%w, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2097
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'delz', FV_Atm(1)%delz)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delz', FV_Atm(1)%delz, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2098
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pt', FV_Atm(1)%pt)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pt', FV_Atm(1)%pt, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2099
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'delp', FV_Atm(1)%delp)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'delp', FV_Atm(1)%delp, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2100
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qvapor', FV_Atm(1)%q(:,:,:,sphu))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qvapor', FV_Atm(1)%q(:,:,:,sphu), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2101
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qliquid', FV_Atm(1)%q(:,:,:,qliq))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qliquid', FV_Atm(1)%q(:,:,:,qliq), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2102
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qice', FV_Atm(1)%q(:,:,:,qice))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qice', FV_Atm(1)%q(:,:,:,qice), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2103
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qrain', FV_Atm(1)%q(:,:,:,rain))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qrain', FV_Atm(1)%q(:,:,:,rain), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2104
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qsnow', FV_Atm(1)%q(:,:,:,snow))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qsnow', FV_Atm(1)%q(:,:,:,snow), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2105
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qgraupel', FV_Atm(1)%q(:,:,:,grpl))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qgraupel', FV_Atm(1)%q(:,:,:,grpl), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2106
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'qcld', FV_Atm(1)%q(:,:,:,qcld))
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'qcld', FV_Atm(1)%q(:,:,:,qcld), ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2107
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ps', FV_Atm(1)%ps)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ps', FV_Atm(1)%ps, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2108
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pe', FV_Atm(1)%pe)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pe', FV_Atm(1)%pe, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2109
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pk', FV_Atm(1)%pk)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pk', FV_Atm(1)%pk, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2110
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'peln', FV_Atm(1)%peln)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'peln', FV_Atm(1)%peln, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2111
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'pkz', FV_Atm(1)%pkz)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'pkz', FV_Atm(1)%pkz, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2112
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'phis', FV_Atm(1)%phis)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'phis', FV_Atm(1)%phis, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2113
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'q_con', FV_Atm(1)%q_con)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'q_con', FV_Atm(1)%q_con, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2114
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'omga', FV_Atm(1)%omga)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'omga', FV_Atm(1)%omga, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2115
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'ua', FV_Atm(1)%ua)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'ua', FV_Atm(1)%ua, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2116
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'va', FV_Atm(1)%va)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'va', FV_Atm(1)%va, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2117
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'uc', FV_Atm(1)%uc)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'uc', FV_Atm(1)%uc, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2118
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'vc', FV_Atm(1)%vc)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'vc', FV_Atm(1)%vc, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2119
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfxd', FV_Atm(1)%mfx)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfxd', FV_Atm(1)%mfx, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2120
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'mfyd', FV_Atm(1)%mfy)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'mfyd', FV_Atm(1)%mfy, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2121
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cxd', FV_Atm(1)%cx)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cxd', FV_Atm(1)%cx, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2122
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'cyd', FV_Atm(1)%cy)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'cyd', FV_Atm(1)%cy, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2123
SELECT CASE ( ppser_get_mode() )
  CASE(0)
    call fs_write_field(ppser_serializer, ppser_savepoint, 'diss_estd', FV_Atm(1)%diss_est)
  CASE(1)
  ...
  CASE(2)
    call fs_read_field(ppser_serializer_ref, ppser_savepoint, 'diss_estd', FV_Atm(1)%diss_est, ppser_zrperturb)
END SELECT
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2124
call fs_disable_serialization()
#endif
#ifdef RUN_PYFV3
      ...
      if (.not. ADIABATIC) then
         ...
         do n=1,STATE%GRID%NQ
            ...
            ! QLIQ
            if (qliq /= -1) then
               if ((qlcnf /= -1) .and. (TRIM(state%vars%tracer(n)%tname) == 'QLCN')) then
                  QLCN_FILLED = .TRUE.
                  nn = nn+1
                  if (state%vars%tracer(n)%is_r4) then
                     state%vars%tracer(n)%content_r4(:,:,:) = FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qliq) * &
            ...
            ! QICE
            if (qice /= -1) then
               if ((qicnf /= -1) .and. (TRIM(state%vars%tracer(n)%tname) == 'QICN')) then
                  QICN_FILLED = .TRUE.
                  nn = nn+1
                  if (state%vars%tracer(n)%is_r4) then
                     state%vars%tracer(n)%content_r4(:,:,:) = FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,qice) * &
...
#ifdef SERIALIZE
! file: /home/fgdeconi/work/git/fp/geos/src/Components/@GEOSgcm_GridComp/GEOSagcm_GridComp/GEOSsuperdyn_GridComp/@FVdycoreCubed_GridComp//FV_StateMod.F90.SER lineno: #2548
...
call ppser_finalize()
   ...
   subroutine fv_getUpdraftHelicity(uh25, uh03, srh01, srh03, srh25)
      ...
      real(FVPRC), allocatable :: ustm(:,:), vstm(:,:)

      integer :: isc, iec, jsc, jec, ng, npz
      integer :: isd, ied, jsd, jed
      ...
      call get_vorticity(isc, iec, jsc, jec, isd, ied, jsd, jed, npz, &
